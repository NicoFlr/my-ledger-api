...
namespace Services.Managers.User
{
    public class UserManager : IUserManager
    {
        ...
        public UserDTO Get(Guid Id)
        {
            try
            {
                Data.Models.User? user = _unitOfWork.GetContext().Users.Include(u=>u.Role).Include(u=>u.Transactions).ThenInclude(t=>t.Category).Where(a => a.Id == Id && a.IsDeleted == false).FirstOrDefault();
                if (user != null)
                {
